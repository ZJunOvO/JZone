...
通用歌曲菜单、合集菜单和资料库新建菜单全部复用 `LiquidGlassMotionContent`。整块材质壳使用 inset 几何执行约 840ms 的 Q 弹，内容从模糊 18px、缩放状态过渡到清晰状态；关闭压缩为约 360ms，材质壳、高光和内容同步淡出。菜单使用 `panel` 几何消除长面板角落放射波纹。
...
先实现现在就听头像 ↔ 个人页头像、Mini 播放器 ↔ 全屏播放页；播放器容器从 Mini 实时边界向上、向两侧铺满，封面、歌名和艺人共用曲线，返回时沿原路径逆向收拢。
...
- [x] **Step 2: 同步 Mini 与全屏状态**

封面、主题背景和播放状态使用同一时间轴驱动，页面展开期间播放进度不得跳变。容器和封面/歌名/艺人已改为单进度可中断动画；Mini DOM 与 SVG 折射材质持续挂载，快速反复开合不会重建材质或发生端点闪跳。
...
新增 `smoke:player-transition`，按多个真实时间点验证容器四边 inset 单调变化、次级控制项延迟显现、封面/歌名/艺人双端矩阵和反向收拢；补充连续快速反向 5 轮、Mini SVG 材质持续性、焦点恢复及独立帧预算探针。
