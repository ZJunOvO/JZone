...
- [ ] 修复 Supabase 合集可见性：统一以 `albums.visibility` 为单一事实来源，废弃旧策略对 `albums.is_public` 的放行。
- [ ] 同步现有合集数据：`is_public = (visibility = 'public')`，避免旧数据继续误导。
- [ ] 增加数据库层同步触发器：后续更新 `visibility` 时自动同步 `is_public`，直到旧字段完全删除。
- [ ] 验证测试账号不能看到非本人 private playlist。
- [ ] 检查 `fetchMyCollections` 是否应显式限制当前用户，避免未来误用。

## P1：上传能力统一

- [ ] 新建 `components/upload/UploadEditor.tsx`，承载文件选择、封面、裁剪、元数据、公开私有、合集选择。
...
- [ ] 将 `UploadModal.tsx` 改成 Modal 壳 + `UploadEditor`。
- [ ] 将 `Upload.tsx` 改成创作工作台壳 + `UploadEditor` + 我的上传管理列表。
- [ ] 删除重复上传逻辑，保留一套字段默认值和错误提示。
...
- [ ] 默认艺人名使用当前用户昵称；如果文件名解析出艺人，则优先文件名。
