...
import type { GlassElementConfig, VectorIconSpec } from './types'
import { wrapText } from './gl-utils'

const drawVectorIcon = (ctx: CanvasRenderingContext2D, icon: VectorIconSpec) => {
  const color = `rgba(${Math.round(icon.color[0] * 255)}, ${Math.round(icon.color[1] * 255)}, ${Math.round(icon.color[2] * 255)}, ${icon.color[3]})`
  ctx.fillStyle = color
  ctx.strokeStyle = color
  ctx.lineWidth = icon.strokeWidth ?? 2
  ctx.lineCap = 'round'
  ctx.lineJoin = 'round'

  if (icon.path) {
    const path = new Path2D(icon.path)
    if (icon.fill === false) ctx.stroke(path)
    else ctx.fill(path)
  }
  ...
  for (const node of icon.nodes ?? []) {
    ...
    if (node.kind === 'circle') {
      ...
      ctx.roundRect(node.x, node.y, node.width, node.height, node.radius ?? 0)
    } else {
      const [first, ...rest] = node.points
      if (!first) continue
      ctx.moveTo(first[0], first[1])
      for (const point of rest) ctx.lineTo(point[0], point[1])
      if (node.kind === 'polygon') ctx.closePath()
    }
    if (icon.fill) ctx.fill()
    else ctx.stroke()
  }
}
