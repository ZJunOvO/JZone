...
const readVideoMetadata = (file: File) => new Promise<{ durationMs: number; poster: File | null }>((resolve, reject) => {
  ...
  video.onloadedmetadata = () => {
    ...
    if (!Number.isFinite(durationMs) || durationMs <= 0) {
      ...
      reject(new Error('视频时长无效，请重新导出后再试'));
      return;
    }
    ...
    video.onseeked = () => {
      try {
        ...
        URL.revokeObjectURL(url);
        resolve({ durationMs, poster: null });
      }
    };
  };
  video.onerror = () => {
    URL.revokeObjectURL(url);
    reject(new Error('当前浏览器无法读取这个视频，请优先使用 MP4 或 WebM'));
  };
  video.src = url;
});
...
export const SongVideoManagerDialog: React.FC<SongVideoManagerDialogProps> = ({ song, onClose }) => {
  ...
  return (
    <motion.div className="fixed inset-0 z-[150] flex items-end justify-center bg-black/72 p-3 backdrop-blur-md sm:items-center" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
      <motion.div className="max-h-[92dvh] w-full max-w-lg overflow-y-auto rounded-[28px] border border-white/10 bg-zinc-950/94 p-5 shadow-2xl" initial={{ y: 28, scale: 0.97 }} animate={{ y: 0, scale: 1 }} exit={{ y: 18, opacity: 0 }} transition={{ type: 'spring', stiffness: 300, damping: 30 }}>
        ...
        <input ref={inputRef} type="file" accept="video/mp4,video/webm,video/quicktime,.m4v" className="hidden" onChange={(event) => { void selectFile(event.target.files?.[0]); event.currentTarget.value = ''; }} />
